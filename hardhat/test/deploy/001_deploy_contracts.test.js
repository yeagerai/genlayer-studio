...
describe("Deploy Script", function () {
  ...
  describe("Deployment Files Verification", function () {
    it("should have all contracts in directories", async function () {
      ...
      for (const contractName of expectedContracts) {
        ...
        const deployContractPath = path.join(
          deployPath,
          `${contractName}.json`,
        );
        expect(
        ...
        const deployData = JSON.parse(
          await fs.readFile(deployContractPath, "utf8"),
        );

        const contractAddress = await contracts[contractName].getAddress();
        expect(
          deployData.address,
          `${contractName} should have valid address in deployments`,
  ...
  describe("Deployment Initialization and Configuration Validation", function () {
    ...
    it("should have initialized GhostFactory properly", async function () {
      const ghostBlueprintAddress =
        await contracts.GhostFactory.ghostBlueprint();
      expect(
        ghostBlueprintAddress,
        "GhostFactory should have been initialized with GhostBlueprint address",
    ...
    it("should have initialized ConsensusMain properly", async function () {
      ...
      expect(
        genManagerAddress,
        "ConsensusMain should have been initialized with GenManager address",
    ...
    it("should have set Acceptance Timeout in ConsensusMain properly", async function () {
      const acceptanceTimeout =
        await contracts.ConsensusMain.ACCEPTANCE_TIMEOUT();
    ...
    it("should have set up validators in MockGenStaking properly", async function () {
      ...
      expect(
        validators,
        "MockGenStaking should have set up validators",
      ).to.deep.equal([
        validator1.address,
        validator2.address,
        validator3.address,
    ...
    it("should have initialized Appeals contract properly", async function () {
      ...
      expect(
        appealsAddress,
        "Appeals contract should have a valid address",
      ).to.not.equal(ZeroAddress);
    });
