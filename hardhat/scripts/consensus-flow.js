...
async function main() {
  ...
  const consensusDataAddress =
    require("../deployments/localhost/ConsensusData.json").address;
  const consensusMain = await hre.ethers.getContractAt(
    "ConsensusMain",
    consensusMainAddress,
  );
  const consensusData = await hre.ethers.getContractAt(
    "ConsensusData",
    consensusDataAddress,
  );
  ...
  const txActivateTransactionInput =
    consensusMain.activateTransaction.getFragment(txId, vrfProofActivate);
  ...
  const leaderAddress = consensusMain.interface.parseLog(
    activationReceipt.logs[0],
  ).args[1];
  ...
  const vrfProofPropose = await generateSignature(
    leader,
    BigInt(await consensusMain.recipientRandomSeed(ghostAddress)),
  );
  ...
  const txCommitVoteInput = consensusMain.commitVote.getFragment(
    txId,
    voteHash3,
    false,
  );
  const lastCommitTx = await consensusMain
    .connect(validator3)
    .commitVote(txId, voteHash3, false);
  ...
  await consensusMain
    .connect(validator1)
    .revealVote(txId, voteHash1, voteType, nonces[0], false);
  await consensusMain
    .connect(validator2)
    .revealVote(txId, voteHash2, voteType, nonces[1], false);
  const revealVoteInput3 = consensusMain.revealVote.getFragment(
    txId,
    voteHash3,
    voteType,
    nonces[2],
  ...
  const revealReceipt3 = await consensusMain
    .connect(validator3)
    .revealVote(txId, voteHash3, voteType, nonces[2], false);
  ...
  const txFinalizeTransactionInput =
    consensusMain.finalizeTransaction.getFragment(txId);
